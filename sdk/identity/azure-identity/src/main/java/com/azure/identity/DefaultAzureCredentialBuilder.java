 ...
 *
 * <p>
 * The {@link DefaultAzureCredential} is appropriate for most scenarios where
 * the application is intended to
 * ultimately be run in Azure. DefaultAzureCredential combines credentials that
 * are commonly used to authenticate when
 * deployed, with credentials that are used to authenticate in a development
 * environment.
 * The {@link DefaultAzureCredential} will attempt to authenticate via the
 * following mechanisms in order.
 * </p>
 *
 * <p>
 * <strong>Sample: Construct DefaultAzureCredential</strong>
 * </p>
 *
 * <p>
 * The following code sample demonstrates the creation of a
 * {@link DefaultAzureCredential}, using
 * the DefaultAzureCredentialBuilder to configure it. Once this credential is
 * created, it may be passed into the
 * builder of many of the Azure SDK for Java client builders as the 'credential'
 * parameter.
 * </p>
 *
 ...
 *
 * <p>
 * <strong>Sample: Construct DefaultAzureCredential with User Assigned Managed
 * Identity </strong>
 * </p>
 *
 * <p>
 * User-Assigned Managed Identity (UAMI) in Azure is a feature that allows you
 * to create an identity in
 * <a href=
 * "https://learn.microsoft.com/en-us/azure/active-directory/fundamentals/">Azure
 * Active Directory (Azure AD)
 * </a> that is associated with one or more Azure resources. This identity can
 * then be used to authenticate and
 * authorize access to various Azure services and resources. The following code
 * sample demonstrates the creation of
 * a {@link DefaultAzureCredential} to target a user assigned managed identity,
 * using the DefaultAzureCredentialBuilder
 * to configure it. Once this credential is created, it may be passed into the
 * builder of many of the
 * Azure SDK for Java client builders as the 'credential' parameter.
 * </p>
 *
 * <!-- src_embed
 * com.azure.identity.credential.defaultazurecredential.constructwithuserassignedmanagedidentity
 * -->
 * 
 * <pre>
...
public class DefaultAzureCredentialBuilder extends CredentialBuilderBase<DefaultAzureCredentialBuilder> {
     ...
     * {@link DefaultAzureCredential}. If unset, the value
     * in the AZURE_TENANT_ID environment variable will be used. If neither is set,
     * the default is null
     * and will authenticate users to their default tenant.
     *
     * @param tenantId the tenant ID to set.
     * @return An updated instance of this builder with the tenant id set as
     *         specified.
     */
     ...
     * @return An updated instance of this builder with the authority host set as
     *         specified.
     */
    ...
    /**
     * Specifies the KeePass database path to read the cached credentials of Azure
     * toolkit for IntelliJ plugin.
     * The {@code databasePath} is required on Windows platform. For macOS and Linux
     * platform native key chain /
     * key ring will be accessed respectively to retrieve the cached credentials.
     *
     * <p>
     * This path can be located in the IntelliJ IDE.
     * Windows: File -&gt; Settings -&gt; Appearance &amp; Behavior -&gt; System
     * Settings -&gt; Passwords.
     * </p>
     *
     * @param databasePath the path to the KeePass database.
     * @throws IllegalArgumentException if {@code databasePath} is either not
     *                                  specified or is empty.
     * @return An updated instance of this builder with the KeePass database path
     *         set as specified.
     */
    ...
    /**
     * Specifies the client ID of user assigned or system assigned identity, when
     * this credential is running
     * in an environment with managed identities. If unset, the value in the
     * AZURE_CLIENT_ID environment variable
     * will be used. If neither is set, the default value is null and will only work
     * with system assigned
     * managed identities and not user assigned managed identities.
     *
     * Only one of managedIdentityClientId and managedIdentityResourceId can be
     * specified.
     *
    ...
    /**
     * Specifies the client ID of Azure AD app to be used for AKS workload identity
     * authentication.
     * if unset,
     * {@link DefaultAzureCredentialBuilder#managedIdentityClientId(String)} will be
     * used.
     * If both values are unset, the value in the AZURE_CLIENT_ID environment
     * variable
     * will be used. If none are set, the default value is null and Workload
     * Identity authentication will not be attempted.
     *
     * @param clientId the client ID
     * @return the DefaultAzureCredentialBuilder itself
     */
    ...
    /**
     * Configure the path to a file containing the federated token that
     * authenticates the identity.
     * The file path is required to authenticate.
     *
     * @param tokenFilePath the path to the file containing the token to use for
     *                      authentication.
     * @return An updated instance of this builder with the tenant id set as
     *         specified.
     */
    ...
    /**
     * Specifies the resource ID of user assigned or system assigned identity, when
     * this credential is running
     * in an environment with managed identities. If unset, the value in the
     * AZURE_CLIENT_ID environment variable
     * will be used. If neither is set, the default value is null and will only work
     ...
     * managed identities and not user assigned managed identities.
     *
     * Only one of managedIdentityResourceId and managedIdentityClientId can be
     * specified.
     *
    ...
    /**
     * Specifies the ExecutorService to be used to execute the authentication
     * requests.
     * Developer is responsible for maintaining the lifecycle of the
     * ExecutorService.
     *
     * <p>
     * If this is not configured, the {@link ForkJoinPool#commonPool() common fork
     * join pool} will be used which is
     * also shared with other application tasks. If the common pool is heavily used
     * for other tasks, authentication
     * requests might starve and setting up this executor service should be
     * considered.
     * </p>
     *
     * <p>
     * The executor service and can be safely shutdown if the TokenCredential is no
     * longer being used by the
     * Azure SDK clients and should be shutdown before the application exits.
     * </p>
     *
     * @param executorService the executor service to use for executing
     *                        authentication requests.
     * @return An updated instance of this builder with the executor service set as
     *         specified.
     */
     ...
     * Add the wildcard value "*" to allow the credential to acquire tokens for any
     * tenant the application is installed.
     *
    ...
    public DefaultAzureCredentialBuilder additionallyAllowedTenants(String... additionallyAllowedTenants) {
        this.additionallyAllowedTenants = IdentityUtil
    ...
    /**
     * Specifies a {@link Duration} timeout for developer credentials (such as Azure
     * CLI) that rely on separate process
     * invocations.
     * 
    ...
    /**
     * Disables the setting which determines whether or not instance discovery is
     * performed when attempting to
     * authenticate. This will completely disable both instance discovery and
     * authority validation.
     * This functionality is intended for use in scenarios where the metadata
     * endpoint cannot be reached, such as in
     * private clouds or Azure Stack. The process of instance discovery entails
     * retrieving authority metadata from
     * https://login.microsoft.com/ to validate the authority. By utilizing this
     * API, the validation of the authority
     * is disabled. As a result, it is crucial to ensure that the configured
     * authority host is valid and trustworthy.
     *
    ...
    private void loadFallbackValuesFromEnvironment() {
        ...
        tokenFilePath = CoreUtils.isNullOrEmpty(tokenFilePath) ? configuration.get(AZURE_FEDERATED_TOKEN_FILE)
                : tokenFilePath;
    }
    ...
    private ArrayList<TokenCredential> getCredentialsChain() {
        ...
        output.add(new ManagedIdentityCredential(managedIdentityClientId, managedIdentityResourceId,
